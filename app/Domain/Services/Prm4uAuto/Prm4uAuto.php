...
class Prm4uAuto extends APrm4uAuto
{
    private $url;
    private $key;
    ...
    public function getStatus($orderId): ExternStatus
    {
        ...
        if (! empty($res['orders'])) {
            ...
            echo "ids: $subOrders";
            echo "received from APrm4u " . json_encode($res) . PHP_EOL;

            try {
                $order = Chunk::where('extern_id', $orderId)
                    ->where('service_class', APrm4uAuto::class)
                    ->firstOrFail()
                    ->compositeOrder;
    ...
    public function charge($orderId, $count, $svcConfig): float
    {
        try {
            ...
            Log::info("[CHUNK_CHARGE] AutoPrm4u could not get charge on order {$orderId}");
