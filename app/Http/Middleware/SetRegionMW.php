...
class SetRegionMW
{
    ...
    const REGION_UKRAINE = 'UKRAINE';
    const REGION_UNKNOWN = 'UNKNOWN';
    const REGION_UZBEKISTAN = 'UZBEKISTAN';
    ...
    public static function getRegion($country): string
    {
        $countryRegionMap = [
            ...
            'UZ' => self::REGION_UZBEKISTAN,    // Узбекистан
            ...
            'BR' => self::REGION_BRAZIL, // Бразилия
            'UA' => self::REGION_UKRAINE,// Украина
            self::LOCALE_UNKNOWN => self::REGION_UNKNOWN,
        ];
    ...
    {
        $regionCurrencyMap = [
            self::REGION_BRAZIL => Transaction::CUR_USD,
            ...
            self::REGION_UKRAINE => Transaction::CUR_RUB,
            self::REGION_UNKNOWN => Transaction::CUR_USD,
            self::REGION_UZBEKISTAN => Transaction::CUR_UZS
        ];
    ...
    public function handle($req, Closure $next)
    {
        ...
        /*
         * Сохраняем локаль пользователя в Request::$localization, т.к. это свойство используется для установки локали в
         * App\Http\Middleware::Localization
         */
        $locale = self::LOCALE_UNKNOWN === $req->country_value ? App::getFallbackLocale() : $req->country_value;
        $req->localization = strtolower($locale);
